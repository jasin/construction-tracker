...
export const useDocumentStore = defineStore('document', () => {
  ...
  async function initializeUserDocumentsSubscription() {
    ...
    try {
      // Subscribe to all documents and filter for user's documents
      userDocumentsUnsubscribe = DocumentRepository.subscribeToDocuments((documents) => {
        // Filter documents uploaded by or reviewed by current user
        userDocuments.value = documents.filter((doc) =>
          doc.uploadedBy === userId || doc.reviewedBy === userId || doc.approvedBy === userId
        )
        userDocumentsLoading.value = false
        userDocumentsInitialized.value = true
        console.log('Document Store: User documents updated, count:', userDocuments.value.length)
      })
    } catch (error) {
  ...
  async function initializeProjectDocumentsSubscription(projectId) {
    ...
    try {
      projectDocumentsUnsubscribe = DocumentRepository.subscribeToDocumentsByProject(projectId, (documents) => {
        projectDocuments.value = documents
        projectDocumentsLoading.value = false
        console.log('Document Store: Project documents updated, count:', documents.length)
      })
    } catch (error) {
  ...
  async function updateDocument(documentId, updates) {
    ...
    try {
      const updatedDocument = await DocumentRepository.updateDocument(documentId, updates)
  ...
  async function updateDocumentStatus(documentId, status, comments = '') {
    ...
    try {
      const updatedDocument = await DocumentRepository.updateDocumentStatus(documentId, status, comments)
